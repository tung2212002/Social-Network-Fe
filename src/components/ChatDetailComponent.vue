    ...
    if (res.status === 200) {
      ...
      if (data && Array.isArray(data.message)) {
        ...
        await nextTick(() => {
          ...
          messageList.value = data.message.map(msgData => {
            let newMessage = {};
            let msgQuote = null;
            let msgReply = null;
          ...
          data.userSeen.map((us) => {
            seenList.value.push({
              ...
              avatar: us.avatar || "https://static.vecteezy.com/system/resources/previews/009/292/244/original/default-avatar-icon-of-social-media-user-vector.jpg",
            })
          })
...
const changeGroupName = async (body) => {
  try {
    ...
    if (res.status === 200) {
      ...
      if (res.message === 'You do not have permission to change this group')
        toast.error('Bạn không đủ quyền', { timeout: 3000 });
      else
        toast.error('Không thể đổi tên nhóm', { timeout: 3000 });
    }
  } catch (err) {
    toast.error('Đã xảy ra lỗi', { timeout: 3000 });
  } finally {
...
//   if (message.groupId === props.chat.groupId) {
//     if (message.msgType !== 'TYPING' && message.msgType !== 'DELETE' && message.msgType !== 'RECALL') {
//       // Thêm tin nhắn vào danh sách nếu loại tin nhắn không phải là "TYPING"
//       let newMessage = {
//         msgId: message.id,
...
//       // Kiểm tra xem tin nhắn có phải là phản hồi không
//       if (message.replyMsgId) {
//         // Tìm tin nhắn gốc (quote) để thiết lập
//         let msgQuote = messageList.value.find(msg => msg.msgId === message.replyMsgId);

//         if (msgQuote) {
//           // Thêm thông tin msgQuote vào tin nhắn mới
//           newMessage.msgQuote = {
//             user: msgQuote.user,
//             msgId: msgQuote.msgId,
//             message: {
//               content: msgQuote.message.content,
//               msgType: msgQuote.message.msgType,
//               createAt: msgQuote.message.createAt,
//             },
//             reactionQuantity: msgQuote.reactionQuantity || 0,
//             images: msgQuote.images || []
//           };
//         }
//       }

//       // Đưa tin nhắn vào danh sách
//       messageList.value.push(newMessage);
//       console.log("Running");

//     } else {
//       // Xử lý khi nhận được thông điệp "TYPING"
//       const existingTyping = typingList.value.find(
//         (t) => t.avatar === message.user.avatar
//       );

//       if (!existingTyping) {
//         typingList.value.push({
//           avatar: message.user.avatar,
//           typingTimeout: null
//         });
//       } else {
//         // Nếu đã có trong danh sách typing, xóa timeout trước đó
//         if (existingTyping.typingTimeout) {
//           clearTimeout(existingTyping.typingTimeout);
//         }
//       }

//       // Thiết lập timeout để tự động xóa khỏi danh sách typing sau 5 giây
//       const timeoutId = setTimeout(() => {
//         const index = typingList.value.findIndex(
//           (t) => t.avatar === message.user.avatar
//         );
//         if (index !== -1) {
...
//       // Cập nhật timeout mới cho người dùng đang gõ
//       const typingUser = typingList.value.find(
//         (t) => t.avatar === message.user.avatar
//       );
//       if (typingUser) {
//         typingUser.typingTimeout = timeoutId;
//       }
//     }
//   }
// };
    ...
    if (res.status === 200) {
      datas.map(data => {
        members.value.push({
          ...
          avatar: data.user.avatar || "https://static.vecteezy.com/system/resources/previews/009/292/244/original/default-avatar-icon-of-social-media-user-vector.jpg",
      ...
      if (datas !== null)
        datas.map((data) => {
          users.push({
            ...
            avatar: data.avatar || "https://static.vecteezy.com/system/resources/previews/009/292/244/original/default-avatar-icon-of-social-media-user-vector.jpg"
          })
        })
...
const setShowOptionsMsg = (msgId) => {
  if (showOptionsMessage.value === true && messageFocusId.value !== msgId)
...
const handlerTag = (value) => {
  ...
  if (atIndex !== -1) {
    const searchQuery = value.substring(atIndex + 1).trim().toLowerCase();

    recommendations.value = members.value.filter(member =>
...
<template>
  <div class="chat-detail">
    ...
    <div class="messages-container">
      <div v-for="(msg, index) in messageList" :key="`${props.chat.groupId}-${msg.msgId}-${index}`" :class="{
        ...
        'current-user-message': msg.user.userId === currentUserId && msg.message.msgType === 'MESSAGE' || msg.message.msgType === 'DELETE' || msg.message.msgType === 'IMAGE',
        'other-user-message': msg.user.userId !== currentUserId,
      }">
        <div class="message-reply message-content" v-if="msg.msgQuote && msg.msgQuote.message.content != ''">{{
          msg.msgQuote.message.content }}
        </div>
        <div class="message-wrapper">
          <div :class="{ 'option-message-container': true, 'option-other-user': msg.user.userId !== currentUserId }"
            v-if="showOptionsMessage && messageFocusId === msg.msgId" v-click-outside="setNoShowOptionMsg">
            <button class=" button-option" v-if="msg.user.userId === currentUserId"
              @click="deleteMessage(msg.msgId)">Xóa tin nhắn</button>
            <button class="button-option" v-if="msg.user.userId === currentUserId" @click="recallMessage(msg.msgId)">Thu
              hồi tin nhắn</button>
            <button class="button-option" @click="replyMessage(msg.msgId, msg.message.content)">Trả lời tin
              nhắn</button>
          </div>
          ...
          <div class="left-message">
            <div :class="{ 'option-message': msg.user.userId === currentUserId }"
              v-if="msg.user.userId === currentUserId && (msg.message.msgType === 'MESSAGE' || msg.message.msgType === 'IMAGE')"
              @click="setShowOptionsMsg(msg.msgId)" style="margin-right: 7px;"><i
                class="fa-regular fa-comment-dots"></i>
            </div>
            <template v-if="msg.message.msgType === 'MESSAGE'">

              <div class="message-content" @mouseenter="setShowReaction(msg.msgId, true)"
                @mouseleave="handleMouseLeave(msg.msgId)">
                <div class="reaction-menu" v-show="showReaction[msg.msgId]">
                  ...
                  <i class="reaction-item fa-regular fa-face-grin-squint-tears"
                    @click="handleReaction(msg.msgId, 'SMILE')"></i>
                </div>
              ...
              <div class="option-message"
                v-if="msg.user.userId !== currentUserId && (msg.message.msgType === 'MESSAGE' || msg.message.msgType === 'IMAGE')"
                @click="setShowOptionsMsg(msg.msgId)" style="margin-left: 7px;">
            ...
            <template v-else>
              <div v-if="msg.images && msg.images.length > 0" :class="[
                'image-container',
                msg.user.userId === currentUserId ? 'image-right' : 'image-left'
              ]">
    ...
    <div v-if="!inGroup" class="not-in-group-message">
  ...
  <div v-if="showOptions" class="options-overlay">
    <div class="options-content" v-click-outside="toggleOptions">
      ...
      <div class="nick-name">
        <input type="text" class="input-nick-name" @change="(e) => { newNickName = e.target.value }"
  ...
  <div v-if="showCallNotification" class="call-notification">
    <div class="notification-content">
      <p v-if="!call">Đang gọi<strong>{{ callerName }}</strong></p>
      <p v-else>Nhận cuộc gọi từ <strong>{{ callerName }}</strong></p>
