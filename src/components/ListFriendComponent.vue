...
const numberFriends = computed(() => friendStore.getNumberFriend);
...
const loadMoreFriends = () => {
    ...
    getFriendListService({
        ...
        .then((res) => {
            if (res.status === 200) {
                if (state.firstLoad) {
                    friendStore.setFriends(res.data.data.friendshipUsers);
                    state.firstLoad = false;
                } else {
                    friendStore.addListFriend(res.data.data.friendshipUsers);
                }
                state.page++;
                if (res.data.data.friendshipUsers.length < state.size) {
                    state.hasMore = false;
                }
                friendStore.setNumberFriend(res.data.data.friendQuantity);
            }
        })
        .catch((err) => {
            console.error(err);
...
const updateFriendStatus = (friendIndex, newStatus) => {
    state.listFriend[friendIndex].status = newStatus;
};
...
<template>
    <div class="friend-list-container">
        <div v-if="numberFriends > 0">
            <span> Danh sách bạn bè </span>
            <span class="font-weight-bold"> {{ numberFriends }} người </span>
        </div>
